...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.createPayment.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.create-payments.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                <label for="amount">{{ trans('cruds.createPayment.fields.amount') }}</label>
                <input type="number" id="amount" name="amount" class="form-control" value="{{ old('amount', isset($createPayment) ? $createPayment->amount : '') }}" step="0.01">
                ...
                <p class="helper-block">
                    {{ trans('cruds.createPayment.fields.amount_helper') }}
                </p>
            </div>
